-- Per-card Ship/Stash choice migration.
-- Run this once in the Supabase SQL editor (Project -> SQL Editor -> New
-- query) against an EXISTING project. Safe to paste and run more than once.
--
-- What this does: fulfillment method (Ship Out vs Stash With Us) moves from
-- a single whole-order column to a per-claim (and per-queue-entry) column,
-- since a cart can now mix Ship and Stash items in one checkout instead of
-- picking one method for everything. The buyer's shipping name/phone/
-- address/zip stay on `orders` as before (unchanged - the destination is
-- the same regardless of which items ship), only `fulfillment_method`
-- itself moves down to `card_claims`/`dibs_queue`.
--
-- This changes place_order()'s signature: it drops the p_fulfillment_method
-- parameter (now a "fulfillment_method" key inside each item object in
-- p_items instead) - update any client code calling it before or
