...
create table cards (
  ...
  -- Whether buyers can Make Offer on this listing - admin-set per card via
  -- InventoryForm, enforced both client-side (CardDetail hides the button)
  -- and server-side (submit_offer rejects it) since the client check alone
  -- is trivially bypassable. Defaults true so existing listings keep
...
-- ---------------------------------------------------------------------------
-- MIGRATION 17: bootstraps supabase_functions.http_request() - the internal
-- plumbing the Supabase Dashboard's "Database Webhooks" UI normally creates
-- automatically the first time it's used, missing on this project (surfaced
-- as "schema supabase_functions does not exist" when creating a webhook
-- there). Needed for app/api/webhooks/notifications/route.ts - a webhook on
-- notifications' Insert event emails a seller (via Resend) for a new offer
-- or new order, on top of the in-app bell, since submit_offer/place_order
-- run as direct client-side RPCs with no Next.js server in that request to
-- call Resend from directly. Safe to run more than once.
-- ---------------------------------------------------------------------------
...
-- actually stop a direct RPC call). Safe to run more than once.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- MIGRATION 19: Live Sales - a super admin running a Facebook Live stream
-- can assign a card straight to a buyer's account (findBuyerByHandle/
-- assignLiveSale/removeLiveSale in app/admin/actions.ts) instead of
-- checking it out under their own account, which is what happened before
-- this existed. is_live_sale marks which card_claims rows came from that
-- flow, so the new /admin/live-sales page can show just those - separate
-- from the regular Sales Log, which still shows every sale regardless of
-- channel. Safe to run more than once.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- MIGRATION: atomic admin claim/stock mutations. Safe to run more than once.
--
-- The buyer-facing mutations above (place_order, cancel_claim, offers) all
-- lock their rows with `for update` inside one function call, so two
-- concurrent buyers can never both succeed against the same stock. The
-- admin-side equivalents in app/admin/actions.ts (confirmPaid, cancelRelist,
-- promoteNextInQueue, assignLiveSale/removeLiveSale, resolveDisputeRestock)
-- didn't have that: they read a claim/card, checked its status in
-- JavaScript, then issued separate .update() calls - two admin actions
-- racing on the same claim (e.g. a double-clicked "Confirm Paid" landing
-- at the same time as a stale second tab's "Cancel & Relist") could both
-- pass the status check before either write landed, leaving a claim marked
-- paid while its stock was also restored as if cancelled. These functions
-- move the check-and-write into one locked transaction, same pattern as
-- cancel_claim below.
-- ---------------------------------------------------------------------------
